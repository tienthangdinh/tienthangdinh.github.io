...
date: 2025-06-25
categories: [Machine Learning, Reinforcement Learning, Model-free Learning]
format:
  html:
    toc: true
    code-fold: true
...
# Function Approximation (parameterized continuous Value)
...
# Policy Gradient Methods (parameterized continuous policy)
...
* **For a policy (e.g., logits in a softmax):**
    For each action $a$, you'd have a separate weight vector $\mathbf{\theta}_a$, and the logits for the policy could be:
    $$
...
The logits for each action are:
* $h(s, a=-1, \mathbf{\theta}) = \mathbf{\theta}_{-1}^T \mathbf{x}(s)$
* $h(s, a=0, \mathbf{\theta}) = \mathbf{\theta}_{0}^T \mathbf{x}(s)$
* $h(s, a=1, \mathbf{\theta}) = \mathbf{\theta}_{1}^T \mathbf{x}(s)$

And the policy probabilities are then given by the softmax function:
$$\pi(a|s, \mathbf{\theta}) = \frac{e^{h(s,a,\mathbf{\theta})}}{\sum_{a'} e^{h(s,a',\mathbf{\theta})}}$$

**4. Update the Policy Parameters ($\mathbf{\theta}$) using Policy Gradients (REINFORCE):**

When we collect a trajectory and compute returns $G_t$, the REINFORCE update rule applies:
$$\mathbf{\theta} \leftarrow \mathbf{\theta} + \alpha G_t \nabla_{\mathbf{\theta}} \ln \pi(a_t|s_t, \mathbf{\theta})$$

Let's look at a specific component of this update, say for $\mathbf{\theta}_{-1}$:
$$\mathbf{\theta}_{-1} \leftarrow \mathbf{\theta}_{-1} + \alpha G_t \nabla_{\mathbf{\theta}_{-1}} \ln \pi(a_t|s_t, \mathbf{\theta})$$
...
## Sum up: REINFORCE Gradient Policy with Baseline
To specify upfront:
...
For $m = 1, \dots, M$:
  ...
  1. Sample: $s_0^m, a_0^m, r_1^m \dots a_{T_m-1}^m, r_{T_m}^m$
  2. For $t = 0, \dots, T_m - 1$:
  $$g_t^m \leftarrow r_{t+1}^m + \gamma r_{t+2}^m + \dots$$
...
The PPO policy objective to maximize is:
