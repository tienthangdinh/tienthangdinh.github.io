...
- Regular Implicit Function: An implicit function f is called regular if its gradient ∇f(x) does not vanish aka = 0 (no cut itself)
...
- The gradient vector ∇f(x) is always orthogonal to the surface at x: n(x) ∝ ∇f(x)
...
- A level set is an entire family of this function: l_λ(x) = f(x) - λ 

6. **How can CSG-operations be expressed in the formalism of implicit functions?**
- CSG on implicit functions (f₁, f₂, etc.), think like what would a position value be:
- f_union(x) = min(f₁(x), f₂(x)).
- f_intersection(x) = max(f₁(x), f₂(x)).
- Difference (A \ B): The set of points in A but not in B. This corresponds to f_difference(x) = max(f₁(x), -f₂(x)).
...
8. **Given an image of a superquadric estimate the exponents 𝑝₁ and 𝑝₂ of the used Minkowski norms!**
- Exponent p₁ controls the shape in the xy-plane (looking down the z-axis).
    - p₁ < 2: The shape is concave or "star-like".
    - p₁ = 2: The cross-section is a perfect circle.
    - p₁ > 2: The cross-section becomes more "square-like".
- Exponent p₂ controls the shape in the rz-plane (the profile along the z-axis).
    - p₂ < 2: The shape is pointy at the poles.
    - p₂ = 2: The profile is circular.
    - p₂ > 2: The profile becomes more "box-like".
...
- The overall implicit function f(x) is defined by summing the potentials from all atoms and subtracting this sum from a flipped  Gaussian value: f(x) = 1/e - Σ h_i(x) (look at that function in the slide you will understand)
...
- Function Value Mapping: combining the scalar values from generic functions: m^F(p) = m(f₁(p), f₂(p), ...):
    - CSG: Union (min), Intersection (max), Difference (max(f₁, -f₂)).
    - Levelset Mapping: m(f, λ) = f - λ
    - Outline Mapping: m(f, λ) = f² - λ² (creates two offset surfaces).
...
- Space Warping: These deform the surface by modifying the space (input coordinate)s before they are evaluated by the function f. The new function is f'(q) = f(w(q)), where w is the inverse warp.
    - Linear Transformations: Affine transformations like rotation, scaling, and shear, represented by a matrix M. The warp is w(q) = M⁻¹q.
...
- Find the inverse p = w(q) = M⁻¹q. (bringing the coordinate system there)
- Then apply the original function: f^w(q) = f(w(q)) = f(M⁻¹q)
- The gradient of the transformed surface is found using the chain rule: ∇f^w = ∇f(p) ⋅ J_w
where ∇f(p) is the gradient of the original function evaluated at the un-warped point p, and J_w is the Jacobian of the warp function. For a linear transformation, the Jacobian is simply the matrix itself, so:
∇f^w(q) = ∇f(M⁻¹q) ⋅ M⁻¹
...
5. **Motivate problems of the least squares fit in the vicinity of outliers, C⁰ and C¹ discontinuities!**
- The standard L₂-norm (least squares) is highly sensitive discontiuinity C0 C2 + outliers
...
- flip criteria
  ...
  - König: idea is simpler curve with less curvature
...
- (∇χ ≈ V) (V are the surface normals)
- (χ(p) = 0) constraints at surface (screened poisson)
- How to do this? we derivate one more to get the Laplace/Poisson equation where we seem to be able to to solve bc we know the divergence inside/outside (divχ = ∇V)
...
- Axis-Angle / Quaternions: rotation by angle α around axis n is identical to a rotation by angle -α around axis -n. (q and -q) represent the exact same 3D rotation. (similar to complex number)
...
- α_i angular offset of the 2 joint axes z_i & z_i-1
- θ_i actual angular change of x_i and x_i-1 on z-1 axe
- a_i common perpendicular length
- To sum up, DH = Rot(θ_i) * Trans(d_i) * Trans(a_i) * Rot(α_i), where the actual rotation is Rot(θ_i) & Trans(d_i)
...
- Revolute -> joint angle θ_i is variable.
...
- For an angle like the link twist α_i, you compute both:
- sin(α_i) = ||ẑ_{i-1} × ẑ_i||
- cos(α_i) = <ẑ_{i-1}, ẑ_i>
- However, the sine term is always positive, restricting the angle to [0, π]. SO that does not actually eliminate the result we want anyway => sign of the cross product of angle by taking a dot product dot producting with a third, orthogonal vector ( x̂_i). 
- The final formula becomes: α_i = sgn(<ẑ_{i-1} × ẑ_i, x̂_i>) * arctan2(||ẑ_{i-1} × ẑ_i||, <ẑ_{i-1}, ẑ_i>)
...
8. **Explain the principle steps of the “Skinning Mesh Animations” (SMA) approach!**
...
11. **Where is room for improvement in the SMA approach that was used in „Fast&efficient skinning of animated meshes“?**
