...
## Dreamer
**Goal:** 2 network to learn: 

* Q-Network (image)->(action): actually the same as Q-Network, but the input is not state, it is image (env observation), Ground Truth taken from table as below (Loss can be MSE)
  $$
  Q(s_t, a_t) \leftarrow Q(s_t, a_t) + \alpha \left[ r_t + \gamma \max_a Q(s_{t+1}, a) - Q(s_t, a_t) \right]
  $$
