...
* Q-Learning: The ground-truth is based on the max of the next step from the table
...
End-to-end learning can potentially capture complex relationships between images and actions, offers simplicity but requires more data and computational resources, while separating perception and control provides interpretability and efficiency but requires explicit transfer between modules and engineering efforts.

## Dreamer
**Goal:** Encode the state into sth informative then act from that latent space (semantically structured packed space)
* everytime acting, bring to a new encoded space, then take that encoded space to continue 2 networks as below.
* Q-Network (encoded latent space)->(action): actually the same as Q-Network, but the input is not state, it is encoded image (env observation), Ground Truth taken from table as below (Loss can be MSE). That is how we could **generalized the STATE**
  $$
  Q(s_t, a_t) \leftarrow Q(s_t, a_t) + \alpha \left[ r_t + \gamma \max_a Q(s_{t+1}, a) - Q(s_t, a_t) \right]
  $$

* Reconstruction-Network (VAE): just to make sure the latent space we learned was correct, reconstruction loss also MSE for each pixel
...
def act(self, state, explore=False): #either random or just take the most rewarded action from Q-network
	...
	def update(self): #update both networks at the same time
        self.update_vae()
		self.update_dqn()
...
for epoch in range(num_epochs):
        for batch in self.replay_buffer:
            ...
            loss.backward()
```
* but, can we use this data to do a reinforcement learning? - No, because we should let RL **explore** to regularize the environment, just learning from this data makes it does not know what to do in unseen state (dsitributional shift), it infact overestimate because it has never seen a bad example before 
* => Dagger

## Offline Reinforcement Learning with BCQ
* the same, use Variational Autoencoder, using 2 networks:
* VAE to propose actions (input: state, action) -> modified action
* modified Q-network to evaluate these actions using expected future reward

Vorgehen:
* act: generates multiple plausible actions (from the VAE), perturbs them slightly (with the Actor), and then chooses the best among those constrained options using its Q-function. This is the "batch-constrained" part in action, preventing the agent from selecting highly uncertain, out-of-distribution actions.
    * at each state, sample 100 candidate actions
    * VAE generates each similar version of them
    * weiterleiten to Actor: pertube actions
    * Critic network Q-value
    * choose the best action from 100

* train:
    * VAE (the core): takes a (state, action) from dataset to reconstruct a recon, then MSE (recon, action), this pack action more semantically in latent space (also use reconstruction loss + KL divergence)
    * Critic: estimate Q-values for (state, action) pairs from the dataset, while avoiding overestimation for out-of-distribution actions
    * Actor: aim to find slightly better actions within the constrained space
